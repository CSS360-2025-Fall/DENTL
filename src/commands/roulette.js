...
function rouletteButtons() {
  const betTypes = [
    ...
    { id: "even", label: "Even (1:1)", style: 3 },
    { id: "odd", label: "Odd (1:1)", style: 3 },
  ];
  ...
  rows.push({
    type: 1,
    components: betTypes.slice(12, 14).map((bt) => ({
      type: 2,
      custom_id: `roulette_type_${bt.id}`,
      style: bt.style,
      label: bt.label,
    ...
    return [
      {
        ...
        components: [
          {
            type: 2,
            custom_id: "roulette_select_dozen_1",
            label: "1st Dozen (1-12)",
            style: 1,
          },
          {
            type: 2,
            custom_id: "roulette_select_dozen_2",
            label: "2nd Dozen (13-24)",
            style: 1,
          },
          {
            type: 2,
            custom_id: "roulette_select_dozen_3",
    ...
    return [
      {
        ...
        components: [
          {
            type: 2,
            custom_id: "roulette_select_column_1",
            label: "Column 1",
            style: 1,
          },
          {
            type: 2,
            custom_id: "roulette_select_column_2",
            label: "Column 2",
            style: 1,
          },
          {
            type: 2,
            custom_id: "roulette_select_column_3",
            label: "Column 3",
            style: 1,
          },
        ],
...
async function sendFollowUpMessage(interaction, content) {
  ...
  await fetch(url, {
    ...
    body: JSON.stringify({
      content: content,
      allowed_mentions: { parse: [] }, // Prevents accidental pings
    }),
  });
}

// Execute game and determine win/loss
async function executeGame(session, userId, betType, selection, interaction) {
...
export async function execute(interaction) {
  ...
  if (sessions.has(userId)) {
    ...
    return {
      ...
      data: {
        content:
          "You already have a Roulette game in progress... Ending past session!",
        flags: 64,
      },
    };
  }
  ...
  sessions.set(userId, { bet: bet, userId: userId });
...
export async function interact(interaction) {
  ...
  for (const [sessionUserId, sess] of sessions.entries()) {
    if (userId !== sessionUserId && interaction.data.custom_id.startsWith('roulette_')) {
      return {
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
        data: { 
          content: "❌ This isn't your game! Start your own with `/roulette`", 
          flags: 64 
        }
      };
    }
  ...
  if (customId.startsWith("roulette_type_")) {
    ...
    const fixedBets = ["even", "odd", "red", "black", "low", "high", "five"];
    if (fixedBets.includes(betType)) {
      return executeGame(session, userId, betType, null, interaction);
    }
    ...
    return {
      type: InteractionResponseType.UPDATE_MESSAGE,
      data: {
        content: `🎰 **${betType.toUpperCase()} Bet (${getPayout(
          betType
        )}:1)\n${getRouletteBoard()}\n**\nSelect your numbers:`,
...
export async function handleModalSubmit(interaction) {
  ...
  if (betType === "single") {
    ...
    // Special cases
    if (num === "0" || num === "00") {
      return executeGame(session, userId, betType, num, interaction);
    }
  ...
  if (betType === "split") {
    ...
    if (numbers.length !== 2 || numbers.some((n) => isNaN(n))) {
      ...
      return {
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
        data: {
          content: '❌ Invalid! Enter two numbers like "5,8".',
          flags: 64,
        },
      };
    }
    ...
    if (!isAdjacent) {
      ...
      return {
        type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
        data: {
          content: "❌ Numbers must be adjacent on the table!",
          flags: 64,
        },
      };
    }
